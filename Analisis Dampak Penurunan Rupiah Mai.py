...
data= {
    ...
    'Kurs_Jual_Per_1_Dollar_Berdasarkan_Data_BI' : [16057.89, 16306.13, 17807.60, 18020.65]
}
...
def halaman_1(event=None):
        ...
        if 'ax_back' in globals() and ax_back is not None:
            ...
            ax_back = None
        plt.clf()
        plt.plot(df['periode'], df['Kurs_Jual_Per_1_Dollar_Berdasarkan_Data_BI'], marker='o')
        plt.title("Perubahan Kurs Jual Dari Tahun 2024 Menuju 2026 Berdasarkan Data BI", pad=15)
        plt.xlabel("Periode")
        plt.ylabel("Kurs Jual Rupiah Per 1 Dollar", labelpad=15)
        plt.ylim(16000, 18500)

        for i in range(4) :
            x = df['periode'][i]
            y = df['Kurs_Jual_Per_1_Dollar_Berdasarkan_Data_BI'][i]
...
def pindah_halaman(event=None):
        ...
        Kurs_Beli_Per_1_Dollar_Berdasarkan_Data_BI = [15898.11, 16143.88, 17630.40, 17841.35]
        plt.plot(periode_baru, Kurs_Beli_Per_1_Dollar_Berdasarkan_Data_BI, marker='o', color='crimson')
        plt.title("Perubahan Kurs Beli Dari Tahun 2024 Menuju 2026 Berdasarkan Data BI", pad=15)
        plt.xlabel("Periode")
        plt.ylabel("Kurs Beli Rupiah Per 1 Dollar", labelpad=15)
        plt.ylim(15500, 18000)
        plt.grid(True)

        for i in range(4) :
            x = periode_baru[i]
            y = Kurs_Beli_Per_1_Dollar_Berdasarkan_Data_BI[i]
            plt.annotate(f"{y}", (x, y), textcoords="offset points", xytext=(0, 10), ha='center')
